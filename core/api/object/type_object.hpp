...
namespace ism
{
	...
	// type check
#define OBJECT_CHECK_TYPE(o) (ism::api::typeof(o).has_feature(ism::TypeFlags_Type_Subclass))

	/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

	// type ref
	MAKE_CUSTOM_REF(TypeObject)
	{
	...
	public:
		...
		> void add_object(Name && name, Value && value) noexcept
		{
			...
			if (api::is_dunder_name(str_name)) { m_ptr->update_slot(str_name); }
