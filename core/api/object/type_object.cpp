...
OBJECT_EMBED(TypeObject, t, TypeFlags_BaseType | TypeFlags_HaveVectorCall | TypeFlags_Type_Subclass)
{
	...
	t.tp_getattro = (getattrofunc)ism::type_getattro;

	t.tp_setattro = (setattrofunc)ism::type_setattro;
...
bool TypeObject::ready()
{
	...
	if (!tp_base && this != *ism::typeof<OBJ>()) { tp_base = ism::typeof<OBJ>(); }
...
bool TypeObject::is_subtype(TYPE value) const
{
	...
	else
	{
		...
		return value.is(ism::typeof<OBJ>());
	}
}
...
Error TypeObject::update_slot(STR name)
{
	...
	case hash("__new__"): { tp_new = (newfunc)[](TYPE type, OBJ args) -> OBJ {
		if (STR_IDENTIFIER(__new__); OBJ f{ type.lookup(&ID___new__) }) { return ism::call_object(f, args); }
		return nullptr;
	}; } break;

	case hash("__del__"): { tp_del = (delfunc)[](Object * obj) -> void {
		if (STR_IDENTIFIER(__del__); OBJ f{ ism::typeof(obj).lookup(&ID___del__) })
		{
		}
	}; } break;
	...
	case hash("__call__"): { tp_call = (binaryfunc)[](OBJ self, OBJ args) -> OBJ {
		if (STR_IDENTIFIER(__call__); OBJ f{ ism::typeof(self).lookup(&ID___call__) }) { return ism::call_object(f, args); }
		return nullptr;
	}; } break;

	case hash("__hash__"): { tp_hash = (hashfunc)[](OBJ self) -> hash_t {
		if (STR_IDENTIFIER(__hash__); OBJ f{ ism::typeof(self).lookup(&ID___hash__) }) { return ism::call_object(f, self).cast<hash_t>(); }
		return 0;
	}; } break;

	case hash("__len__"): { tp_len = (lenfunc)[](OBJ self) -> ssize_t {
		if (STR_IDENTIFIER(__len__); OBJ f{ ism::typeof(self).lookup(&ID___len__) }) { return ism::call_object(f, self).cast<ssize_t>(); }
		return -1;
	}; } break;

	case hash("__repr__"): { tp_repr = (reprfunc)[](OBJ self) -> STR {
		if (STR_IDENTIFIER(__repr__); OBJ f{ ism::typeof(self).lookup(&ID___repr__) }) { return ism::call_object(f, self); }
		return nullptr;
	}; } break;

	case hash("__str__"): { tp_str = (reprfunc)[](OBJ self) -> STR {
		if (STR_IDENTIFIER(__str__); OBJ f{ ism::typeof(self).lookup(&ID___str__) }) { return ism::call_object(f, self); }
		return nullptr;
	}; } break;

	case hash("__get__"): { tp_descr_get = (descrgetfunc)[](OBJ self, OBJ obj, OBJ type) -> OBJ {
		if (STR_IDENTIFIER(__get__); OBJ f{ ism::typeof(self).lookup(&ID___get__) }) { /* TODO */ }
		return nullptr;
	}; } break;

	case hash("__set__"): { tp_descr_set = (descrsetfunc)[](OBJ self, OBJ obj, OBJ type) -> Error {
		if (STR_IDENTIFIER(__set__); OBJ f{ ism::typeof(self).lookup(&ID___set__) }) { /* TODO */ }
...
{
	if (ism::typeof<OBJ>() != base || (tp_flags & TypeFlags_HeapType) && !tp_new)
	{
		tp_new = base->tp_new;
	}
	...
	//	if (mro.contains(ism::typeof<TYPE>())) { flag_set(tp_flags, TypeFlags_Type_Subclass); }
	//	else if (mro.contains(ism::typeof<INT>())) { flag_set(tp_flags, TypeFlags_Int_Subclass); }
	//	else if (mro.contains(ism::typeof<FLT>())) { flag_set(tp_flags, TypeFlags_Float_Subclass); }
	//	else if (mro.contains(ism::typeof<STR>())) { flag_set(tp_flags, TypeFlags_Str_Subclass); }
	//	else if (mro.contains(ism::typeof<LIST>())) { flag_set(tp_flags, TypeFlags_List_Subclass); }
	//	else if (mro.contains(ism::typeof<DICT>())) { flag_set(tp_flags, TypeFlags_Dict_Subclass); }
	//}
}
