...
namespace ism
{
	...
	public:
		api::function_record * m_record{};

		NODISCARD auto & operator*() const { return const_cast<api::function_record &>(*m_record); }
	...
	protected:
		void initialize_generic(api::function_record * record_in, std::type_info const * const * info_in, size_t argc_in, bool prepend = false);

		template <class Func, class Return, class ... Args, class ... Extra
		> void initialize(Func && func, Return(*)(Args...), Extra && ... extra)
		{
			api::function_record * rec{ memnew(api::function_record{}) };
			...
			if (sizeof(Capture) <= sizeof(rec->data))
			{
				...
				{
					rec->free_data = [](api::function_record * f) { ((Capture *)&f->data)->~Capture(); };
				}
			}
			else
			{
				...
				rec->free_data = [](api::function_record * f) { memdelete((Capture *)f->data[0]); };
			}

			// convert function arguments and perform the actual function call
			rec->impl = [](api::function_call & call) -> OBJ
			{
				api::argument_loader<Args...> args{};
...
#define OBJECT_CHECK_CPPFUNCTION(o) (ism::api::isinstance<ism::CPP_FUNCTION>(o))
	...
	// cppfunction ref
	MAKE_CUSTOM_REF(CppFunctionObject)
	{
...
NODISCARD inline ism::OBJ ism::CustomRef<ism::FunctionObject>::cpp_function() const
{
