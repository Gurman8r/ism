...
#define OBJECT_CHECK_MODULE(o) (isinstance<ModuleRef>(o))
	...
	{
		CUSTOM_REF(ModuleRef, OBJECT_CHECK_MODULE);

	public:
		template <class Value = ObjectRef
		> void add_object(cstring name, Value && value, bool overwrite = false) { VALIDATE(m_ptr)->add_object(name, FWD(value), overwrite); }

		template <class Func, class ... Extra
		> ModuleRef & def(cstring name, Func && func, Extra && ... extra) noexcept { return VALIDATE(m_ptr)->def(name, FWD(func), FWD(extra)...), (*this); }

		ModuleRef def_submodule(cstring name) { return VALIDATE(m_ptr)->def_submodule(name); }

		void reload() { VALIDATE(m_ptr)->reload(); }
	};
}
