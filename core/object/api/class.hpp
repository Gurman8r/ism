...
namespace ism
{
	...
	public:
		...
		{
			return def_property_static(name, fget, fset, api::attr::is_method(*this), FWD(extra)...);
		}
		...
		{
			if (fget) { api::attr::process_attributes<Extra...>::init(***fget, FWD(extra)...); }
	
			if (fset) { api::attr::process_attributes<Extra...>::init(***fset, FWD(extra)...); }
