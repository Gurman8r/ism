...
namespace ISM
{
	struct Quat
	{
		...
		constexpr Quat(float_t x = 0.f, float_t y = 0.f, float_t z = 0.f, float_t w = 0.f) noexcept : m_data{ x, y, z, w } {}

		constexpr Quat(Vec4 const & value) : m_data{ value } {}

		constexpr Quat(Vec4 && value) noexcept : m_data{ std::move(value) } {}

		constexpr Quat(Quat const & other) : m_data{ other.m_data } {}

		constexpr Quat(Quat && other) noexcept : m_data{} { this->swap(std::move(other)); }
		...
		constexpr void swap(Quat & other) noexcept
		{
...
#ifdef ISM_QUAT_EXTRA
		ISM_QUAT_EXTRA
#endif // ISM_QUAT_EXTRA
