#include <ext/mono/register_mono_types.hpp>
#include <ext/mono/csharp.hpp>
...
void initialize_mono_module(void * user, IsmExtensionInitializationLevel level)
{
	...
	cs_language = memnew(CSharpLanguage); get_scripting()->register_language(cs_language);
}
...
void finalize_mono_module(void * user, IsmExtensionInitializationLevel level)
{
	...
	get_scripting()->unregister_language(cs_language); memdelete(cs_language);
