...
namespace ism
{
	ISM_API_FUNC(ImGuiContext *) ImGui_Initialize();

	ISM_API_FUNC(void) ImGui_Finalize(ImGuiContext * ctx = nullptr);

	ISM_API_FUNC(void) ImGui_BeginFrame(ImGuiContext * ctx = nullptr);

	ISM_API_FUNC(void) ImGui_RenderDrawData(ImDrawData * draw_data);

	ISM_API_FUNC(void) ImGui_EndFrame(ImGuiContext * ctx = nullptr);
}
