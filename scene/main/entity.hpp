...
namespace Ism
{
	...
	public:
		...
		{
			ASSERT(m_data.tree);
			C & c{ m_data.tree->m_ecs.emplace<C>(m_entity_id, FWD(args)...) };
			m_data.tree->on_component_added<C>(*this, c);
			return c;
		}
		...
		{
			return VALIDATE(m_data.tree)->m_ecs.get<C...>(m_entity_id);
		}

		template <class ... C
		> bool has_component() const
		{
			return VALIDATE(m_data.tree)->m_ecs.has<C...>(m_entity_id);
		}

		template <class ... C
		> void remove_component()
		{
			VALIDATE(m_data.tree)->m_ecs.remove<C...>(m_entity_id);
		}
	};
